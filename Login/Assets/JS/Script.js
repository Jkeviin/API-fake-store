...
btn_listo_register.addEventListener('click', function(){
    ...
    if(correo_register.value == "" || usuario_register.value == "" || nombre_register.value == "" || apellido_register.value == "" || contraseña_register.value == "" || contraseña_confirm.value == ""){
        ...
        if(contraseña_register.value != contraseña_confirm.value){
            ...
            msg += "<br>- The password must be at least 6 characters long";
        }if(contraseña_register.value.length > 20){
            msg += "<br>- Password must be less than 20 characters";
        }if(contraseña_register.value.search(/[a-z]/i) < 0){
            msg += "<br>- The password must have at least one lowercase letter.";
        }if(contraseña_register.value.search(/[A-Z]/i) < 0){
            msg += "<br>- The password must have at least one capital letter.";
        }if(contraseña_register.value.search(/[0-9]/i) < 0){
            msg += "<br>- The password must have at least one number";
        }if(contraseña_confirm.value == ""){
            msg += "<br>- Confirm password";
        }if(correo_register.value.search(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/) == -1){
            msg += "<br>- The email is invalid";
        }
        if(correo_register.value.length > 50){
            msg += "<br>- The email must be less than 50 characters";
        }if(usuario_register.value.length < 6){
            msg += "<br>- The user must have at least 6 characters";
        }if(usuario_register.value.length > 20){
            msg += "<br>- User must be less than 20 characters";
        }if(nombre_register.value.search(/[0-9]/i) > 0){
            msg += "<br>- The name cannot contain numbers";
        }if(apellido_register.value.search(/[0-9]/i) > 0){
            msg += "<br>- The last name cannot contain numbers";
        }
    }if(msg != ""){
    ...
    if(msg != ""){
        ...
        .then(response=>{
            if(response.ok){
                ...
                console.clear();
                text_modal.innerHTML = "The user or password is incorrect";
                modalError.style.display = "block";
                throw new Error("Error desconocido");
            }
        })
        .then(json => {
            document.querySelector('.modal__paragraph').innerHTML = "A token has been generated for you";
            document.querySelector('.modal__title').innerHTML = "You started with SUCCESS!";
            modal.classList.add('modal--show');
            console.log(json);
        })
        .catch(error => console.log('ERROR ---->', error))
